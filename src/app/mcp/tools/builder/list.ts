...
export const listBuildersTool: Tool = {
  ...
  description: 'List all builders in Komodo. Builders are build environments used by Builds to create Docker images.',
  schema: z.object({}),
  handler: async (_args, { client, abortSignal }) => {
    ...
    const list =
      builders.map((b) => `• ${b.name} (${b.id}) - Tags: ${b.tags?.join(', ') || 'none'}`).join('\n') ||
      'No builders found.';
