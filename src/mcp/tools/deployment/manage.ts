...
export const getDeploymentInfoTool: Tool = {
  ...
  handler: async (args, { client, abortSignal }) => {
    const komodoClient = requireClient(client, 'komodo_get_deployment_info');
    const result = await wrapApiCall(
      `get deployment info for '${args.deployment}'`,
      () => komodoClient.deployments.get(args.deployment, { signal: abortSignal }),
      abortSignal,
    );
    return {
      content: [
...
- labels: Docker labels like "traefik.enable=true"`,
  ...
  handler: async (args, { client, abortSignal }) => {
    const komodoClient = requireClient(client, 'komodo_create_deployment');
    ...
    /* v8 ignore start - both branches are valid input formats */
    if (args.image) {
    ...
    const result = await wrapApiCall(
      `create deployment '${args.name}'`,
      () => komodoClient.deployments.create(args.name, deploymentConfig, { signal: abortSignal }),
      abortSignal,
    );
    return {
      content: [
...
- Enable auto-update: { auto_update: true }`,
  ...
  handler: async (args, { client, abortSignal }) => {
    const komodoClient = requireClient(client, 'komodo_update_deployment');
    const result = await wrapApiCall(
      `update deployment '${args.deployment}'`,
      () => komodoClient.deployments.update(args.deployment, args.config, { signal: abortSignal }),
      abortSignal,
    );
    return {
      content: [
...
export const deleteDeploymentTool: Tool = {
  ...
  description:
    'Delete (unregister) a Komodo deployment. This removes the deployment configuration but does not affect running containers.',
  schema: z.object({
    deployment: z.string().describe(PARAM_DESCRIPTIONS.DEPLOYMENT_ID),
  }),
  handler: async (args, { client, abortSignal }) => {
    const komodoClient = requireClient(client, 'komodo_delete_deployment');
    const result = await wrapApiCall(
      `delete deployment '${args.deployment}'`,
      () => komodoClient.deployments.delete(args.deployment, { signal: abortSignal }),
      abortSignal,
    );
    return {
      content: [
