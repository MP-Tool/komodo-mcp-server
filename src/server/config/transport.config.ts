 ...
 *
 * @module server/config/transport
 */
 ...
 * createAllowedHosts(config); // ['localhost', '127.0.0.1', '[::1]', 'localhost:3000', ...]
 *
 * // With custom MCP_ALLOWED_HOSTS
 * // MCP_ALLOWED_HOSTS=myapp.example.com,api.example.com
 * createAllowedHosts(config); // ['myapp.example.com', 'api.example.com']
 * ```
 */
export function createAllowedHosts(config: FrameworkEnvConfig): string[] {
...
/**
 * Creates allowed origins list for CORS validation.
 ...
 */
export function createAllowedOrigins(config: FrameworkEnvConfig): string[] {
  const port = config.MCP_PORT;
  // prettier-ignore
  const defaults = [
    `http://localhost:${port}`,
...
export const TRANSPORT_CONFIG = {
  ...
  session: {
    ...
    CLEANUP_INTERVAL_MS: SESSION_CLEANUP_INTERVAL_MS,
    KEEP_ALIVE_INTERVAL_MS: SESSION_KEEP_ALIVE_INTERVAL_MS,
    MAX_MISSED_HEARTBEATS: SESSION_MAX_MISSED_HEARTBEATS,
    MAX_COUNT: SESSION_MAX_COUNT,
    LEGACY_SSE_MAX: LEGACY_SSE_MAX_SESSIONS,
  },
} as const;
