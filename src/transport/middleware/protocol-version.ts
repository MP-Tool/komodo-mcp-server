...
export function validateProtocolVersion(req: Request, res: Response, next: NextFunction): void {
    ...
    if (!protocolVersion) {
        ...
        logProtocolEvent(`No MCP-Protocol-Version header, assuming ${FALLBACK_PROTOCOL_VERSION} for backwards compatibility`);
        (req.headers as any)['mcp-protocol-version'] = FALLBACK_PROTOCOL_VERSION;
        next();
        return;
    }
